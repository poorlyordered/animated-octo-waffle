...
test('renders operations health surface without execution controls or secrets', async ({ page }, testInfo) => {
  ...
  await expectVisibleText(page, 'Operations health filters and saved views organize browser-visible summaries only.');
  const filters = page.getByLabel('Operations health filters');
  await expect(filters.locator('#operations-warning-filter')).toHaveValue('all');
  await expect(filters.locator('#operations-worker-status-filter')).toHaveValue('all');
  await expect(filters.locator('#operations-worker-secret-filter')).toHaveValue('all');
  await expect(filters.locator('#operations-health-saved-view')).toHaveValue('');
  await filters.locator('#operations-warning-filter').selectOption('warning');
  await expectVisibleText(page, '1 of 2');
  await expect(page.getByText('Live Netlify, EVE SSO provider, MongoDB backup/index/access')).toHaveCount(0);
  await filters.locator('#operations-worker-status-filter').selectOption('blocked');
  await filters.locator('#operations-worker-secret-filter').selectOption('missing');
  await expectVisibleText(page, '1 of 5');
  await filters.getByRole('button', { name: 'Save view' }).click();
  await expect(filters.locator('#operations-health-saved-view')).toHaveValue('warning:blocked:missing');
  await filters.locator('#operations-warning-filter').selectOption('all');
  await filters.locator('#operations-worker-status-filter').selectOption('all');
  await filters.locator('#operations-worker-secret-filter').selectOption('all');
  await expectVisibleText(page, '2 of 2');
  await expectVisibleText(page, '5 of 5');
  await filters.locator('#operations-health-saved-view').selectOption('warning:blocked:missing');
  await expect(filters.locator('#operations-warning-filter')).toHaveValue('warning');
  await expect(filters.locator('#operations-worker-status-filter')).toHaveValue('blocked');
  await expect(filters.locator('#operations-worker-secret-filter')).toHaveValue('missing');
  await expectVisibleText(page, '1 of 2');
  await expectVisibleText(page, '1 of 5');
  await filters.getByRole('button', { name: 'Delete view' }).click();
  await expect(filters.locator('#operations-health-saved-view option')).toHaveCount(1);
  const workerReadiness = page.getByLabel('Worker readiness');
  await expect(workerReadiness.getByText('Opportunity ingestion worker callbacks', { exact: true })).toBeVisible();
  await expect(workerReadiness.getByText('Retry worker callbacks', { exact: true })).toHaveCount(0);
  await expect(page.getByLabel('Operations health').getByRole('button', { name: /dispatch|execute|retry|fetch|write/i })).toHaveCount(0);
  await assertNoBrowserDiagnostics();
});
